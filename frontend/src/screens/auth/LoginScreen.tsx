...
import { View, StyleSheet, TextInput, Text, ActivityIndicator, Alert, TouchableOpacity } from "react-native";
...
export const LoginScreen: React.FC<LoginScreenProps> = ({ role }) => {
  ...
  const { login, loading, biometricsAvailable, hasPendingBiometric, unlockWithBiometrics } = useAuth();
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>();

  const handleSubmit = async () => {
    setError(null);
    const result = await login({ username, password, expectedRole: role, totpCode: totp || undefined });
    if (!result.success) {
  ...
  const handleBiometric = async () => {
    ...
    if (!success) {
      Alert.alert("Authentication failed", "We could not verify your biometrics. Try again or sign in with password.");
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <DashboardTile title="Use microphone to fill form" subtitle="Speak your username and password" onPress={() => {}} />
      <TextInput
        style={styles.input}
      ...
      <View style={styles.inputRow}>
        ...
        >
          <Ionicons name={passwordVisible ? "eye" : "eye-off"} size={24} color={palette.textSecondary} />
        </TouchableOpacity>
      </View>
      <TextInput
        style={styles.input}
      ...
      />
      <VoiceButton label="Have a token?" onPress={() => navigation.navigate("PasswordResetConfirm")} />
      <VoiceButton
        label="Switch Role"
