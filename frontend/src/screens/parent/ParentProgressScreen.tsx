...
export const ParentProgressScreen: React.FC = () => (
  <ScrollView contentContainerStyle={styles.container}>
    ...
    <Text style={styles.subtitle}>Tap a subject to hear voice summaries or share updates with teachers.</Text>
    {subjects.map((subject) => (
      <View key={subject.name} style={styles.card}>
        <Ionicons name="ribbon" size={28} color={palette.success} />
        <View style={styles.cardBody}>
          <Text style={styles.cardTitle}>{subject.name}</Text>
          <Text style={styles.cardMeta}>Score {subject.score}% • {subject.attendance}</Text>
